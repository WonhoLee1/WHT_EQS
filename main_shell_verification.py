...
class EquivalentSheetModel:
    ...
    def optimize(self, opt_config, loss_weights, 
        ...
        def loss_fn(p_scaled, fixed_p_scaled, scales):
            ...
            p_fem = {k: v.flatten() for k, v in combined_phys.items() if k != 'pz'}
            ...
            for i, case in enumerate(self.cases):
                ...
                scale_R = jnp.mean(tgt_R_sums) + 1e-6
            ...
            return total_loss, {'Total': total_loss, 'Disp': l_static, 'Strs': l_stress, 'Strn': l_strain, 'Engy': l_energy, 'Reac': l_reaction, 'Freq': l_freq, 'Mode': l_mode, 'Mass': l_mass, 'Reg': l_reg, 'f1_hz': f1_hz}
        ...
        print(f"{'Iter':<5} | {'Total_Norm':<10} | {'Disp':<9} | {'Strs':<9} | {'Strn':<9} | {'Engy':<9} | {'Reac':<9} | {'Freq':<9} | {'Mode':<9} | {'Mass':<9} | {'Freq1':<6}")
        print("-" * 128)
        ...
        for i in range(max_iterations):
            ...
            if i % 10 == 0: print(f"{i:<5d} | {val:<10.2e} | {metrics['Disp']:<9.2e} | {metrics['Strs']:<9.2e} | {metrics['Strn']:<9.2e} | {metrics['Engy']:<9.2e} | {metrics['Reac']:<9.2e} | {metrics['Freq']:<9.2e} | {metrics['Mode']:<9.2e} | {metrics['Mass']:<9.2e} | {metrics['f1_hz']:<6.2f}")
    ...
    def plot_optimization_history(self):
        ...
        all_keys = ['Total', 'Disp', 'Strs', 'Strn', 'Engy', 'Reac', 'Freq', 'Mode', 'Mass', 'Reg']
    ...
    def verify(self):
        ...
        opt_params_h = {
            't': griddata(pts_low, np.array(self.optimized_params['t']).flatten(), (Xh, Yh), method='cubic').flatten(),
            'rho': griddata(pts_low, np.array(self.optimized_params['rho']).flatten(), (Xh, Yh), method='cubic').flatten(),
            'E': griddata(pts_low, np.array(self.optimized_params['E']).flatten(), (Xh, Yh), method='cubic').flatten(),
            'z': griddata(pts_low, np.array(self.optimized_params.get('pz', jnp.zeros_like(self.optimized_params['t']))).flatten(), (Xh, Yh), method='cubic').flatten()
        }
...
if __name__ == '__main__':
    ...
    weights = {
        ...
        'mode': 0.001,           # [Gentle] Mode matching (MAC) - Start small to avoid shock
    ...
    try:
        ...
        weights_stage1['mode'] = 0.1  # 낮은 가중치로 Shock 방지
        
        best_params_s1 = model.optimize(opt_config, weights_stage1, 
                                        use_smoothing=False, 
                                        use_strain_energy=True,      # pz 미분 경로가 살아있는 유일한 경로
                                        use_surface_stress=False,    # shell_solver의 stress 계산은 미분 체인 단절 → 사용 불가
                                        use_surface_strain=False,    # 동일 이유로 비활성화
                                        use_mass_constraint=True, 
                                        mass_tolerance=0.05,
                                        max_iterations=120,    # Stage 1 반복횟수
                                        use_early_stopping=True, 
                                        early_stop_patience=100, 
                                        early_stop_tol=1e-8,
                                        learning_rate=0.5,
        ...
        #weights_stage2['mode'] = 1.   # MAC 가중치 크게 상향
        ...
        model.optimize(opt_config_stage2, weights_stage2, 
                       ...
                       use_mass_constraint=True, 
                       mass_tolerance=0.05,
                       max_iterations=50,    # Fine-tuning은 짧게
                       use_early_stopping=True, 
                       early_stop_patience=50, 
                       early_stop_tol=1e-8,
                       learning_rate=0.1,     # LR도 줄여서 안정성 확보
                       num_modes_loss=5,
                       min_bead_width=10.0,
                       mac_search_window=5)   # 순서가 섞여도 찾을 수 있도록 넉넉히
