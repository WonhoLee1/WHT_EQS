...
def get_thickness_field(X, Y, Lx=1000.0, Ly=400.0, pattern_str="A", base_t=1.0, bead_t=2.0):
    """
    Generate thickness field based on a pattern string.
    Supported characters: 'A', 'B', 'C', and ' ' (space).
    The plate width Lx is divided evenly among the characters.
    """
    num_chars = len(pattern_str)
    char_width = Lx / num_chars
    
    # Initialize thickness map with base thickness
    thickness_map = jnp.full_like(X, base_t)
    
    # Stroke width in normalized coordinates (approx 0.08 of char width)
    # But we want constant physical width? 
    # Let's say physical width ~80mm. 
    # In normalized coords u (0-1), width is 80 / char_width
    w_physical = 60.0 # mm
    
    def dist_segment(px, py, x1, y1, x2, y2):
        # Point (px,py) to segment (x1,y1)-(x2,y2) distance squared
        # Coords are normalized 0-1
        # BUT aspect ratio matters. char_width vs Ly.
        # Let's work in Physical Local Coordinates (0~char_width, 0~Ly)
        
        # x1, y1 are normalized input (0-1). Convert to physical.
    ...
    for i, char in enumerate(pattern_str):
        ...
        # Determine bead thickness for this character
        if isinstance(bead_t, dict):
            current_bead_t = bead_t.get(char, base_t)
        else:
            current_bead_t = bead_t
        ...
        # Define Strokes for each char (start_u, start_v) -> (end_u, end_v)
        strokes = []
        if char == 'A':
            # Padding: 0.2 to 0.8
            strokes = [
                ...
                (0.35, 0.5, 0.65, 0.5) # Middle -
            ]
        elif char == 'B':
            strokes = [
                (0.2, 0.1, 0.2, 0.9), # Vertical |
                (0.2, 0.9, 0.7, 0.9), # Top -
                (0.2, 0.5, 0.6, 0.5), # Mid -
                (0.2, 0.1, 0.7, 0.1), # Bot -
                (0.7, 0.9, 0.8, 0.7), # Top Curve 1
                (0.8, 0.7, 0.6, 0.5), # Top Curve 2
                (0.6, 0.5, 0.8, 0.3), # Bot Curve 1
                (0.8, 0.3, 0.7, 0.1), # Bot Curve 2
            ]
        elif char == 'C':
            strokes = [
                (0.25, 0.2, 0.25, 0.8), # Vertical |
                (0.25, 0.8, 0.7, 0.8),  # Top -
                (0.25, 0.2, 0.7, 0.2),  # Bot -
                # Optional serifs or curve approx? Keep simple box C
            ]
        ...
        # Update thickness map for this character's strokes
        thickness_map = jnp.where(region_mask & is_stroke, current_bead_t, thickness_map)
