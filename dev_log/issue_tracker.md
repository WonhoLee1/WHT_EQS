...
| #010 | 2026-04-16 | 모드 정보 매핑 시 인덱스 오류 | **RESOLVED** | 타겟 생성 시의 모드 개수와 최적화 설정 개수가 다를 때 발생하는 `IndexError` 방지를 위해 실제 로드된 데이터 기준 자동 클램핑(Clamping) 구현. |
| #011 | 2026-04-17 | main_shell_opt.py 및 핵심 솔버 파일 손실 | **RESOLVED** | GitHub(origin/main)에서 main_shell_opt.py, solver.py 등 핵심 의존성 파일을 복구하여 시스템 가동 상태 복원. |
