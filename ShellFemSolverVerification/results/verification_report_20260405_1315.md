...
> 생성일시: **2026-04-05 13:15**  
> 단위계: mm / MPa (N/mm²) / tonne  
> 사용 솔버: `ShellFemSolver/shell_solver.py`

---

## 1. 검증 개요

Solver 수치 신뢰성을 **표준 FEM 패치 테스트**로 검증합니다.  
쉘이론(Kirchhoff 판 이론, CST/DKT 요소)에 기반한 해석해와  
FEM 수치해를 비교하여 응력·변형률·변위 필드의 정확도를 정량 평가합니다.

| 구분 | 내용 |
|------|------|
| 요소 유형 | CQUAD4 (Q4), CTRIA3 (T3 / DKT) |
| 자유도 | 절점당 6-DOF: [u, v, w, θx, θy, θz] |
| 응력 회복 | B-matrix 기반 (CST membrane + DKT bending) |
| 이론 기준 | Kirchhoff 판 이론, Navier 급수해 |

---

## 2. 테스트 결과

| 테스트명 | 요소 | 비교 물리량 | 이론값 | FEM값 | 오차 (%) | 허용 (%) | 결과 |
|----------|------|------------|--------|-------|----------|----------|------|
...
| Bending Patch (T3-DKT) | T3 | κx 평균 곡률 | 0.000416 | -7.9508e-19 | 100.000 | 1.0 | ❌ FAIL |
|  | | *κx_std=3.015e-03 (균일도 오차 374486227919355328.0000%)* | | | | | |
...
> **참고**: 패치 테스트 통과 = solver가 쉘이론에 부합하는 기본 물리 거동을 올바르게 재현함.
> 상세 이론: Batoz & Bathe (1980) DKT, Bletzinger et al. (2000) DSG3