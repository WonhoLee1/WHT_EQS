...
# ──────────────────────────────────────────────────────────────────────────────
#  물리적 응력 회복 유틸리티 (B-matrix 기반)
# ──────────────────────────────────────────────────────────────────────────────

@partial(jit, static_argnames=['mode'])
def get_constitutive_matrix(E, nu, t, mode='membrane'):
    """Plane-stress constitutive matrix."""
    if mode == 'membrane': # C matrix for stress
        factor = E / (1.0 - nu**2)
        return factor * jnp.array([[1.0, nu, 0.0],
                                    [nu, 1.0, 0.0],
                                    [0.0, 0.0, 0.5*(1.0-nu)]])
    else: # D matrix for moment (unit width)
...
def recover_stress_tria_membrane(u, nodes, trias, E, nu):
    """CST 요소를 이용한 막 응력 회복 (vmap 호환)."""
    def get_element_stress_inner(tri_idx):
        node_indices = trias[tri_idx]
        tri_nodes = nodes[node_indices]
        u_el = u.reshape(-1, 6)[node_indices][:, :2].flatten()
        ...
        # Constitutive
        fac = E / (1.0 - nu**2)
        C_mat = fac * jnp.array([[1.0, nu, 0.0], [nu, 1.0, 0.0], [0.0, 0.0, 0.5*(1.0-nu)]])
        
        eps_vec = B_mat @ u_el
        return eps_vec # Pure Strain

    return vmap(get_element_stress_inner)(jnp.arange(len(trias)))

def recover_curvature_tria_bending(u, nodes, trias, E, nu, t):
    """DKT 요소를 이용한 굽심 곡률 및 모멘트 회복."""
    def get_element_curvature_inner(tri_idx):
        ix = trias[tri_idx]
        tri_nodes = nodes[ix]
        # u_b = [w1, tx1, ty1, w2, tx2, ty2, w3, tx3, ty3]
        ...
        tx = u_b[:, 1]; ty = u_b[:, 2] # local rotations theta_x, theta_y
        # kx = -d(theta_y)/dx, ky = d(theta_x)/dy, kxy = ...
        kx_val = -inv_area * (b1*ty[0] + b2*ty[1] + b3*ty[2])
        ky_val = inv_area * (c1*tx[0] + c2*tx[1] + c3*tx[2])
        kxy_val = -inv_area * (c1*ty[0] + c2*ty[1] + c3*ty[2]) + inv_area * (b1*tx[0] + b2*tx[1] + b3*tx[2])
        
        return jnp.array([kx_val, ky_val, kxy_val])

    return vmap(get_element_curvature_inner)(jnp.arange(len(trias)))

def recover_stress_quad_membrane(u, nodes, quads, E, nu):
    """Q4 요소를 이용한 막 응력 회복 (중앙점)."""
    def get_element_stress(q_idx):
        ix = quads[q_idx]
        xe, ye = nodes[ix, 0], nodes[ix, 1]
        ue = u.reshape(-1, 6)[ix][:, :2].flatten()
        
        # 중앙점 (xi=0, eta=0)
        xi, et = 0.0, 0.0
        GN = 0.25 * jnp.array([[-(1-et), (1-et), (1+et), -(1+et)],
                               [-(1-xi), -(1+xi), (1+xi), (1-xi)]])
        J = GN @ jnp.stack([xe, ye], axis=1)
        invJ = jax.scipy.linalg.inv(J)
        DN = invJ @ GN
        
        B = jnp.zeros((3, 8))
        B = B.at[0, 0::2].set(DN[0])
        B = B.at[1, 1::2].set(DN[1])
        B = B.at[2, 0::2].set(DN[1])
        B = B.at[2, 1::2].set(DN[0])
        
        eps = B @ ue
        C = get_constitutive_matrix(E, nu, 1.0, 'membrane')
        return C @ eps
        
    return vmap(get_element_stress)(jnp.arange(len(quads)))

@jit
def compute_von_mises(s):
    """s = [sx, sy, txy]"""
    return jnp.sqrt(s[0]**2 - s[0]*s[1] + s[1]**2 + 3*s[2]**2)

@jit
def compute_principal_stresses(s):
    """s = [sx, sy, txy] -> (s1, s2, angle)"""
    avg = (s[0] + s[1]) / 2.0
    dif = (s[0] - s[1]) / 2.0
    radius = jnp.sqrt(dif**2 + s[2]**2)
    angle = 0.5 * jnp.arctan2(2*s[2], s[0]-s[1])
    return avg + radius, avg - radius, angle
...
class ShellFEM:
    # (Redundant compute_field_results removed. See implementation at end of file)
    ...
    def assemble_sparse(self, params):
        """Helper method to explicitly call sparse assembly without passing the boolean flag."""
    ...
    # ── 정적/고유값 솔버 (High-Level API) ─────────────────────────────────
    def solve_static(self, params, F, prescribed_dofs, prescribed_vals, sparse=True):
        """
        통합 정적 해석 인터페이스.
        params: 물성치 사전
        F: 전체 하중 벡터 (N*6)
        prescribed_dofs: 구속된 DOF 인덱스 배열
        prescribed_vals: 구속된 DOF의 변위 값
        """
        K_s, M_s = self.assemble(params, sparse=sparse)
        
        # 전체 DOF에서 구속된 DOF를 제외한 자유 DOF 계산
        total_range = jnp.arange(self.total_dof)
        free_dofs = jnp.setdiff1d(total_range, jnp.array(prescribed_dofs))
        
        if sparse:
            # Scipy Sparse 인덱싱을 위해 NumPy 배열로 변환 필수
            fd_np = np.array(free_dofs)
            pd_np = np.array(prescribed_dofs)
            pv_np = np.array(prescribed_vals)
            return self.solve_static_sparse(K_s, F, fd_np, pd_np, pv_np)
        else:
            return self.solve_static_partitioned(K_s, F, free_dofs, prescribed_dofs, prescribed_vals)
    ...
    def solve_static_sparse(self, K_s, F, free_dofs, prescribed_dofs, prescribed_vals):
        """Solve static problem using scipy.sparse.linalg.spsolve (CPU)."""
        from scipy.sparse.linalg import spsolve
        import numpy as np
        
        K_ff = K_s[free_dofs, :][:, free_dofs]
        K_fp = K_s[free_dofs, :][:, prescribed_dofs]
        
        rhs = np.array(F[free_dofs]) - K_fp.dot(np.array(prescribed_vals))
        u_f = spsolve(K_ff, rhs)
        
        u = np.zeros(self.total_dof)
        u[np.array(free_dofs)] = u_f
        u[np.array(prescribed_dofs)] = np.array(prescribed_vals)
        return jnp.array(u)
    ...
    def compute_field_results(self, u, params, K=None):
        """[ULTIMATE] 수치적 안정성이 강화된 후처리 엔진."""
        E_nodes = params.get('E', 210000.0); nu = 0.3; t_nodes = params.get('t', 1.0)
        u_j = jnp.array(u).reshape(-1, 6)
        nodes_j = jnp.array(self.nodes)
        
        # 1. Trias (CST + DKT)
        num_t = len(self.trias) if self.trias is not None else 0
        if num_t > 0:
            curv_t = recover_curvature_tria_bending(u_j.flatten(), nodes_j, self.trias, E_nodes.mean(), nu, t_nodes.mean())
            eps_m_t = recover_stress_tria_membrane(u_j.flatten(), nodes_j, self.trias, 1.0, nu)
            eps_t = eps_m_t + curv_t * (t_nodes.mean() / 2.0)
        else:
            eps_t = jnp.zeros((0, 3))

        # 2. Quads (Mindlin)
        num_q = len(self.quads) if self.quads is not None else 0
        if num_q > 0:
            curv_q = recover_curvature_quad_bending(u_j.flatten(), nodes_j, self.quads, E_nodes.mean(), nu, t_nodes.mean())
            eps_m_q = recover_stress_quad_membrane(u_j.flatten(), nodes_j, self.quads, 1.0, nu)
        ...
        def calc_stress(eps):
            if len(eps) == 0: return jnp.zeros((0,3))
            pre = E_nodes.mean() / (1.0 - nu**2)
            s_x = pre * (eps[:,0] + nu * eps[:,1])
            s_y = pre * (eps[:,1] + nu * eps[:,0])
            s_xy = (E_nodes.mean() / (2.0 * (1.0 + nu))) * eps[:,2]
            return jnp.stack([s_x, s_y, s_xy], axis=1)
            
        sig_el = calc_stress(eps_el)
        
        # 4. Indices & Smoothing
        vm_el = jnp.sqrt(jnp.maximum(sig_el[:,0]**2 - sig_el[:,0]*sig_el[:,1] + sig_el[:,1]**2 + 3*sig_el[:,2]**2, 0.0))
        avg_e = (eps_el[:, 0] + eps_el[:, 1]) / 2.0
        r_e = jnp.sqrt(jnp.maximum(((eps_el[:, 0] - eps_el[:, 1]) / 2.0)**2 + (eps_el[:, 2] / 2.0)**2, 0.0))
        eps_major_el = avg_e + r_e
        
        stress_node = jnp.zeros(self.num_nodes); strain_major_node = jnp.zeros(self.num_nodes); count_node = jnp.zeros(self.num_nodes)
        
        if num_t > 0:
            stress_node = stress_node.at[self.trias.flatten()].add(jnp.repeat(vm_el[:num_t], 3))
            strain_major_node = strain_major_node.at[self.trias.flatten()].add(jnp.repeat(eps_major_el[:num_t], 3))
            count_node = count_node.at[self.trias.flatten()].add(3.0)
        if num_q > 0:
            stress_node = stress_node.at[self.quads.flatten()].add(jnp.repeat(vm_el[num_t:], 4))
            strain_major_node = strain_major_node.at[self.quads.flatten()].add(jnp.repeat(eps_major_el[num_t:], 4))
            count_node = count_node.at[self.quads.flatten()].add(4.0)
            
        safe_count = jnp.maximum(count_node, 1.0)
        return {
            'stress_vm': stress_node / safe_count,
            'stress_vm_el': vm_el,
            'strain_max_principal': strain_major_node / safe_count,
            'strain_major_el': eps_major_el,
            'u_mag': jnp.linalg.norm(u_j[:, :3], axis=1)
        }

    def compute_max_surface_stress(self, u, params, K=None):
        return self.compute_field_results(u, params, K=K)['stress_vm']

    def compute_max_surface_strain(self, u, params, K=None):
        E = params.get('E', 210000.0)
        return self.compute_max_surface_stress(u, params, K=K) / E.mean()

def recover_curvature_quad_bending(u, nodes, quads, E, nu, t):
    """Q4 Mindlin 요소를 이용한 굽힘 곡률/모멘트 회복 (정밀 자코비안 기반)."""
    def get_element_curvature(q_idx):
        ix = quads[q_idx]; q_nodes = nodes[ix]
        # 로컬 좌표계 (단순화: 첫 두 노드로 x축 정렬)
        v12 = q_nodes[1]-q_nodes[0]; e1 = v12/jnp.linalg.norm(v12).clip(1e-12)
        v14 = q_nodes[3]-q_nodes[0]; nrm = jnp.cross(e1, v14)
        e3 = nrm / jnp.linalg.norm(nrm).clip(1e-12)
        e2 = jnp.cross(e3, e1)
        # 2D 좌표 (4, 2)
        p2d = jnp.stack([jnp.dot(q_nodes-q_nodes[0], e1), jnp.dot(q_nodes-q_nodes[0], e2)], axis=1)
        
        # 중앙점 (xi=0, eta=0) 형상함수 미분
        ...
        # Bending DOFs: tx, ty (Local)
        # u_b = [w1, tx1, ty1, ...] -> (4, 3) 
        # Global rotations -> Local rotations
        u_global = u.reshape(-1, 6)[ix]
        # thy_local 는 굽힘 곡률에 기여 (dw/dx ~ thy)
        # tx_loc/ty_loc conversion using e1, e2
        tx_g = u_global[:, 3]; ty_g = u_global[:, 4]
        tx_l = jnp.dot(jnp.stack([tx_g, ty_g], axis=1), jnp.array([e1[0], e1[1]])) # Simplified mapping
        ty_l = jnp.dot(jnp.stack([tx_g, ty_g], axis=1), jnp.array([e2[0], e2[1]]))
        
        # Curvature k = [d(thy)/dx, -d(thx)/dy, d(thy)/dy - d(thx)/dx]
...
def recover_stress_quad_membrane(u, nodes, quads, E, nu):
    ...
    def get_element_stress(q_idx):
        ...
        u_m_g = u.reshape(-1, 6)[ix][:, :2] # u,v Global
        # Approximate local mapping (assuming in-plane)
        u_l = jnp.dot(u_m_g, jnp.array([e1[0], e1[1]])) 
        v_l = jnp.dot(u_m_g, jnp.array([e2[0], e2[1]]))
        
        eps_x = jnp.dot(dn_dx, u_l)
        eps_y = jnp.dot(dn_dy, v_l)
        eps_xy = jnp.dot(dn_dy, u_l) + jnp.dot(dn_dx, v_l)
        
        return jnp.array([eps_x, eps_y, eps_xy]) # Pure Strain
    
    return vmap(get_element_stress)(jnp.arange(len(quads)))

    def compute_field_results(self, u, params, K=None):
        """[CORE] B-matrix 기반 정밀 필드 사출 및 노드 기반 보간."""
        E_nodes = params.get('E', 210000.0); nu = 0.3; t_nodes = params.get('t', 1.0)
        u_j = jnp.array(u).reshape(-1, 6)
        nodes_j = jnp.array(self.nodes)
        
        # 1. Strain/Curvature Recovery (Elemental)
        # Trias
        if len(self.trias) > 0:
            # Curvatures (3, n_tria)
            curv_t = recover_curvature_tria_bending(u_j.flatten(), nodes_j, self.trias, E_nodes.mean(), nu, t_nodes.mean())
            # Surface Strains at z = t/2 (Top)
            eps_b_t = curv_t * (t_nodes.mean() / 2.0) # k * z
            # Membrane Strains (Simplified CST)
            eps_m_t = recover_stress_tria_membrane(u_j.flatten(), nodes_j, self.trias, 1.0, nu) # B@u
            # Total Strains (Top)
            eps_t = eps_m_t + eps_b_t
        else:
            eps_t = jnp.zeros((0, 3))
        ...
        if len(self.quads) > 0:
            curv_q = recover_curvature_quad_bending(u_j.flatten(), nodes_j, self.quads, E_nodes.mean(), nu, t_nodes.mean())
            # We need to get membrane strain correctly too
            # For simplicity, reuse recover_stress_quad_membrane logic but for strain (E=1.0)
            eps_m_q = recover_stress_quad_membrane(u_j.flatten(), nodes_j, self.quads, 1.0, nu)
            eps_b_q = curv_q * (t_nodes.mean() / 2.0)
            eps_q = eps_m_q + eps_b_q
        else:
            eps_q = jnp.zeros((0, 3))
        
        # Combine elemental strains
        eps_el = jnp.concatenate([eps_t, eps_q])
        
        # 2. Stress Calculation (Elemental)
        def calc_stress(eps):
            pre = E_nodes.mean() / (1.0 - nu**2)
            s_x = pre * (eps[:,0] + nu * eps[:,1])
            s_y = pre * (eps[:,1] + nu * eps[:,0])
        ...
        # 3. Principal Strains (Major/Minor)
        # eps = [ex, ey, gxy] -> (e1, e2) = (ex+ey)/2 +/- sqrt(((ex-ey)/2)^2 + (gxy/2)^2)
        avg_e = (eps_el[:, 0] + eps_el[:, 1]) / 2.0
        r_e = jnp.sqrt(((eps_el[:, 0] - eps_el[:, 1]) / 2.0)**2 + (eps_el[:, 2] / 2.0)**2)
        eps_major_el = avg_e + r_e
        eps_minor_el = avg_e - r_e
        
        # 4. Von-Mises Stress (Elemental)
        vm_el = jnp.sqrt(sig_el[:,0]**2 - sig_el[:,0]*sig_el[:,1] + sig_el[:,1]**2 + 3*sig_el[:,2]**2)
        
        # 5. Nodal Averaging (Smoothing)
        stress_node = jnp.zeros(self.num_nodes)
        strain_major_node = jnp.zeros(self.num_nodes)
        count_node = jnp.zeros(self.num_nodes)
        
        # Trias
        if len(self.trias) > 0:
            stress_node = stress_node.at[self.trias.flatten()].add(jnp.repeat(vm_el[:len(self.trias)], 3))
            strain_major_node = strain_major_node.at[self.trias.flatten()].add(jnp.repeat(eps_major_el[:len(self.trias)], 3))
            count_node = count_node.at[self.trias.flatten()].add(1.0)
            
        # Quads
        if len(self.quads) > 0:
            off = len(self.trias)
            stress_node = stress_node.at[self.quads.flatten()].add(jnp.repeat(vm_el[off:], 4))
            strain_major_node = strain_major_node.at[self.quads.flatten()].add(jnp.repeat(eps_major_el[off:], 4))
            count_node = count_node.at[self.quads.flatten()].add(1.0)
        ...
        return {
            ...
            'strain_max_principal': strain_major_node / safe_count,
            'strain_major_el': eps_major_el,
            'strain_minor_el': eps_minor_el,
            'u_mag': jnp.linalg.norm(u_j[:, :3], axis=1)
        }

    def compute_max_surface_stress(self, u, params, K=None):
        res = self.compute_field_results(u, params, K=K)
        return res['stress_vm']

    def compute_max_surface_strain(self, u, params, K=None):
        stress = self.compute_max_surface_stress(u, params, K=K)
        E = params.get('E', 210000.0)
